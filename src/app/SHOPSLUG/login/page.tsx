...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      ...
      // Na custom domeni URL je čist (/), na path-baziranom je /slug/
      const host = window.location.hostname
      const isCustomDomain = !['nibis-webshop.vercel.app', 'localhost', '127.0.0.1'].includes(host) && !host.endsWith('.vercel.app')
      router.push(isCustomDomain ? '/' : (shopSlug ? '/' + shopSlug + '/' : '/'))
    }
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
          ...
          <p className="text-center text-sm text-gray-500 mt-4">
            Nemate račun?{' '}
            <Link href={href("/register")} className="text-[var(--brand)] hover:underline font-medium">
              Registrujte se
            </Link>
          </p>
        </div>
